...
- **Statut** : CORRIGÉ (CI vert confirmé, propagé à multi-user-full ; 2026-07-02 propagé aussi au job E2E Coverage : misc-state-handlers `updateDonationLink` charge le profil + poll, delegation-handlers poll l'enregistrement du handler + poll l'effet au lieu d'un waitForTimeout(200) fixe)
...
### ERR-169 — Deploy spothitch.com bloqué 2 mois : job multi-user téléchargeait le build standard (navigateur ne joignait pas l'émulateur)
- **Date** : 2026-07-02
- **Gravité** : CRITIQUE (aucun déploiement sur spothitch.com depuis le 30/04/2026 — main rouge en continu, deploy `needs: [..., e2e-multi-user]`)
- **Description** : Les 4 phases multi-user impaires (1,3,5,7) échouaient de façon déterministe. Symptômes : phase 1 `expect(uid).toBeTruthy()` échoue ; phases 3/5/7 `expect(exists).toBe(true)` / `expect(state).toBe(true)` (lectures Firestore après DM, contact SOS, gamification) échouent. Log navigateur : `SDK login failed (auth/network-request-failed), using localStorage fallback`. Les phases paires passaient (pas d'assertion dépendante d'un vrai UID/Firestore). La fausse piste initiale était un `digest-mismatch` sur l'étape Download build (symptôme, pas la cause).
- **Cause racine** : le job `e2e-multi-user` téléchargeait l'artefact `dist` STANDARD (`VITE_FIREBASE_EMULATOR` seul), or seul `VITE_FIREBASE_EMULATOR_E2E=true` débloque l'émulateur dans le CSP (vite.config.js) + skip Messaging (firebase.js) côté navigateur. Résultat : le navigateur ne pouvait PAS joindre l'émulateur (bien que l'émulateur tourne et soit joignable en curl depuis le shell) → login SDK échoue → fallback localStorage → pas de vrai UID et écritures Firestore perdues. En plus, le UID synthétique du fallback était écrasé par les cycles `setState` de l'app avant lecture par `getCurrentUid`.
- **Correction** : (1) le job `e2e-multi-user` build désormais son PROPRE dist avec `VITE_FIREBASE_EMULATOR_E2E=true` + seed les comptes (même pattern que le job `e2e-firebase` déjà vert), au lieu de télécharger le dist standard ; émulateurs auth+firestore+**storage**. (2) `firebase-helpers.js` : le fallback stocke le UID dans une clé dédiée `__e2e_fallback_uid` que l'app ne touche jamais, et `getCurrentUid()` la lit en dernier recours (durcissement, prouvé localement : phase 1 échoue sans, passe avec).
- **Leçon** : Tout job E2E qui exige des lectures/écritures Firebase DEPUIS LE NAVIGATEUR doit utiliser le build `VITE_FIREBASE_EMULATOR_E2E=true` (pas seulement démarrer l'émulateur côté runner). Démarrer l'émulateur ne suffit pas si le navigateur n'est pas câblé pour l'atteindre. Un `expect(...).toBe(true)` sur une lecture Firestore qui échoue en CI = suspecter d'abord le câblage navigateur→émulateur, pas le test. Vérifier `gh run list --branch=main` régulièrement : main était rouge depuis 2 mois sans que le deploy ne parte.
- **Fichiers** : .github/workflows/ci.yml (job e2e-multi-user), e2e/firebase-helpers.js
- **Suite (tests obsolètes)** : le fix émulateur a fait passer les phases 1/2/4/6/8. Restaient 11 tests OBSOLÈTES (phases 3/5/7) référençant des handlers renommés/supprimés depuis 2 mois (jamais exercés car l'émulateur était injoignable → pourris en silence). Corrigés d'après le code réel + prouvés en local (11/11 verts) : `sendDirectMessage`→`sendDirectMessageTo`, `unblockUser`→`confirmUnblockUser`, `reportUser`→`openReport`, `triggerSOSAlert`→`triggerSOS`, `sosAddContact`→`addEmergencyContact`, `sosRemoveContact`→`removeEmergencyContact`, `openCompanionSearch`→`showCompanionSearchView` (charger Social.js via l'onglet), clé admin `adminTab`→`adminActiveTab`, `showAddFriend`→assert `socialSubTab='messagerie'`, `loadAdminReports`→ouvrir le panel d'abord (lazy), `closeNearbyFriends`→radar beta-gated donc close est un no-op (on vérifie que l'app reste fonctionnelle). Test `sosSetChannel` SUPPRIMÉ (feature retirée : tous les canaux SOS partent simultanément). Leçon bonus : un test qui ne tourne jamais (skip/faux-vert) pourrit ; tout job E2E doit RÉELLEMENT s'exécuter pour rester à jour.
- **Fichiers (tests)** : e2e/multi-user-phase3-social.spec.js, e2e/multi-user-phase5-sos.spec.js, e2e/multi-user-phase7-radar.spec.js
- **Statut** : CORRIGÉ localement (émulateur prouvé phase 1 ; 11 tests obsolètes 11/11 verts en local via fallback) — validation finale sur CI main (job multi-user = main-only)
