  ...
  test('DM received in real-time by second browser', async ({ browser }) => {
    ...
    try {
      ...
      await bobPage.evaluate(async (cid) => {
        ...
        window.__unsub = onSnapshot(collection(getDb(), 'conversations', cid, 'messages'), (snap) => {
      ...
      await page.evaluate(async (cid) => {
        ...
        const msgs = await getDocs(collection(getDb(), 'conversations', cid, 'messages'))
    ...
    try {
      await bobPage.evaluate(async (testUid) => {
        ...
        window.__unsubFr = onSnapshot(
          query(collection(getDb(), 'friendRequests'), where('to', '==', testUid)),
          (snap) => {
            snap.docChanges().forEach(change => {
      ...
      await page.evaluate(async (rid) => {
        await window.__fb.deleteDoc(window.__fb.doc(window.__fb.getDb(), 'friendRequests', rid))
      }, reqId)
    } finally {
      await bobCtx.close()
    }
