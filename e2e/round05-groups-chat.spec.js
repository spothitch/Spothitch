...
test.describe('R05-01 Group conversation', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    for (const key of ['alice', 'bob', 'charlie']) {
      await triggerModuleLoad(sessions[key].page, 'social').catch(() => {})
    }
  })
  ...
  test('alice creates group with bob and charlie', async () => {
    ...
    if (typeof groupId === 'string' && !groupId.startsWith('error:')) {
      const groupDoc = await sessions.bob.page.evaluate(async (id) => {
        ...
        const snap = await getDoc(doc(getDb(), 'groupConversations', id))
    ...
    await snap(sessions.alice.page, PHASE, 'R05-01-group-created', 'after')
  })

  test('alice sends group message → bob and charlie receive', async () => {
    const aliceUid = sessions.alice.uid
    const bobUid = sessions.bob.uid
    const charlieUid = sessions.charlie.uid
    ...
    const result = await sessions.alice.page.evaluate(async ({ aliceUid, bobUid, charlieUid }) => {
      try {
        ...
        const msgRef = await addDoc(collection(db, 'groupConversations', groupRef.id, 'messages'), {
    ...
    if (result.groupId) {
      ...
      const bobMsg = await sessions.bob.page.evaluate(async ({ groupId, msgId }) => {
        const { getDb, doc, getDoc } = window.__fb
        const snap = await getDoc(doc(getDb(), 'groupConversations', groupId, 'messages', msgId))
        return snap.exists() ? snap.data() : null
      }, result)
    ...
    await snap(sessions.bob.page, PHASE, 'R05-01-group-msg-bob', 'after')
    await snap(sessions.charlie.page, PHASE, 'R05-01-group-msg-charlie', 'after')
  })

  test('charlie leaves group → members array updated', async () => {
    const aliceUid = sessions.alice.uid
    const bobUid = sessions.bob.uid
    const charlieUid = sessions.charlie.uid
    ...
    if (groupId) {
      ...
      await sessions.charlie.page.evaluate(async ({ groupId, charlieUid }) => {
        ...
        await updateDoc(doc(getDb(), 'groupConversations', groupId), {
    ...
    await snap(sessions.charlie.page, PHASE, 'R05-01-charlie-leaves', 'after')
  })
})
...
test.describe('R05-02 Zone chat', () => {
  ...
  test('alice sends message in FR zone chat → bob sees it', async () => {
    const aliceUid = sessions.alice.uid
    ...
    if (typeof msgId === 'string' && !msgId.startsWith('error:')) {
      const msg = await sessions.bob.page.evaluate(async (id) => {
        const { getDb, doc, getDoc } = window.__fb
        const snap = await getDoc(doc(getDb(), 'countryChats', 'FR', 'messages', id))
        return snap.exists() ? snap.data() : null
      }, msgId)
    ...
    await snap(sessions.bob.page, PHASE, 'R05-02-zone-chat', 'after')
  })
})
...
test.describe('R05-03 Report', () => {
  test('openReport + submitCurrentReport creates report doc', async ({ browser }) => {
    ...
    await snap(sessions.alice.page, PHASE, 'R05-03-report', 'after')
    await closeSessions(sessions)
  })
})
...
test.describe('R05-04 XSS in group', () => {
  test('XSS in group name is stored safely', async ({ browser }) => {
    ...
    await snap(sessions.alice.page, PHASE, 'R05-04-xss-group', 'after')
    await closeSessions(sessions)
  })
})
