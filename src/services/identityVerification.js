...
export function renderVerificationStatus() {
 ...
 >
 <span class="${isComplete ? '' : 'grayscale'}">${level.icon}</span></div>
 <div class="text-xs ${isComplete ? 'text-white' : 'text-slate-400'}">${level.name.split(' ')[0]}</div></div>
 `;
 }).join('')}
 </div>
 ...
 <span>${t('trustScoreLabel') || 'Score de confiance'}</span></div>
 ...
 <span class="text-purple-300 font-medium">Verification complete ! Tu es 100% verifie.</span></div>
 `}
 </div>
 `;
}
