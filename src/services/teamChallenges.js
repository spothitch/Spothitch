...
export function renderTeamDashboard(state) {
 ...
 <p class="text-white/70">${team.members.length} ${(t('teamMembers') || 'membre{s}').replace('{s}', team.members.length > 1 ? 's' : '')}</p></div>
 <button
 ...
 <div class="text-sm text-slate-400">${t('teamStatKmTraveled') || 'km parcourus'}</div></div>
 <div class="bg-dark-card rounded-xl p-4">
 <div class="text-2xl font-bold text-emerald-400">${team.stats?.spotsValidated || 0}</div>
 <div class="text-sm text-slate-400">${t('teamStatSpotsValidated') || 'spots validés'}</div></div>
 <div class="bg-dark-card rounded-xl p-4">
 <div class="text-2xl font-bold text-purple-400">${team.stats?.countriesVisited?.length || 0}</div>
 <div class="text-sm text-slate-400">${t('teamStatCountriesVisited') || 'pays visités'}</div></div>
 <div class="bg-dark-card rounded-xl p-4">
 <div class="text-2xl font-bold text-amber-400">${team.completedChallenges?.length || 0}</div>
 <div class="text-sm text-slate-400">${t('teamStatChallengesCompleted') || 'défis terminés'}</div></div></div>
 ...
 <div class="text-xs text-slate-400">${challenge.description}</div></div></div>
 <div class="space-y-2">
 <div class="flex justify-between text-sm">
 <span class="text-slate-400">${t('teamChallengeProgress') || 'Progression'}</span>
 <span class="font-medium">${challenge.progress}/${challenge.target} ${challenge.unit}</span></div>
 ...
 ${icon("user", "w-4 h-4")}
 <span>${memberId === team.leader ? icon('crown', 'w-3 h-3 inline text-amber-400') + ' ' : ''}${t('teamMember') || 'Membre'}</span></div>
 `
 )
 .join('')}
 </div></div></div>
