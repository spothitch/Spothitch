...
export async function createGroupConversation(name, memberIds, icon = 'users') {
  ...
  memberProfiles[user.uid] = {
    ...
    avatar: user.photoURL || getState().avatar || 'thumbs-up',
  }
  memberIds.forEach(uid => {
    const f = friends.find(fr => fr.id === uid)
    if (f) memberProfiles[uid] = { name: f.name, avatar: f.avatar || 'thumbs-up' }
  })
...
export async function sendGroupConversationMessage(groupId, text) {
  ...
  const senderAvatar = user.photoURL || state.avatar || 'thumbs-up'
...
export async function addMemberToGroupConversation(groupId, userId) {
  ...
  if (friend) updates[`memberProfiles.${userId}`] = { name: friend.name, avatar: friend.avatar || 'thumbs-up' }
