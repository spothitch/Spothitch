...
export function renderPlanner(state) {
 ...
 <p class="text-slate-400 text-sm">${t('planNextTrip') || 'Planifie ton prochain voyage en autostop'}</p></div>
 ...
 <p class="mt-2">${t('addStartCity') || 'Ajoute une ville de départ'}</p></div>
 `
 ...
 />
 <div id="city-suggestions" class="absolute top-full left-0 right-0 z-10 mt-1 hidden"></div></div>
 ...
 <div class="px-4 mb-4">
 <div id="planner-map" class="h-64 rounded-xl overflow-hidden bg-dark-secondary"></div></div>
 ` : ''}
 ...
 <p class="mt-2 text-sm">${t('noTrips')}</p></div>
 `
 : `
 <div class="space-y-2">
...
function renderTripStep(step, index, totalSteps) {
 ...
 <div class="text-slate-400 text-xs truncate">${step.fullName || ''}</div></div>
...
function renderActiveTripDetails(trip) {
 ...
 <div class="text-xs text-slate-400">Distance</div></div>
 <div class="text-center">
 <div class="text-2xl font-bold text-white">${formatDuration(trip.totalDuration)}</div>
 <div class="text-xs text-slate-400">En voiture</div></div>
 <div class="text-center">
 <div class="text-2xl font-bold text-white">${trip.estimatedDays}j</div>
 <div class="text-xs text-slate-400">Estimé stop</div></div></div>
...
export function renderSavedTripDetail(tripId) {
 ...
 <p class="text-slate-400 text-xs">Créé le ${new Date(trip.createdAt).toLocaleDateString()}</p></div>
 ...
 <div class="text-xs text-slate-400">Distance totale</div></div>
 <div class="text-center">
 <div class="text-xl font-bold text-white">${trip.steps.length}</div>
 <div class="text-xs text-slate-400">Étapes</div></div>
 <div class="text-center">
 <div class="text-xl font-bold text-white">${trip.estimatedDays}j</div>
 <div class="text-xs text-slate-400">Durée estimée</div></div></div>
 ...
 <div class="text-slate-400 text-xs">${step.fullName || ''}</div></div></div>
 `).join('')}
 </div></div>
 ...
 <div class="text-slate-400 text-xs">✓${spot.userValidations || 0} • ${spot.avgWaitTime}min</div></div></div>
 `).join('')}
 </div></div>
 `).join('')}
 </div></div>
