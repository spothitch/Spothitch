 ...
 * Pure render functions extracted from Profile.js to reduce file size.
 */
...
export function renderLanguagePickerModal(state) {
 ...
 <button onclick="closeLanguagePicker()" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center" aria-label="${escapeHTML(t('close') || 'Close')}">${icon('x', 'w-4 h-4')}</button></div>
 <div class="p-3 border-b border-white/10">
 <input type="text" placeholder="${t('searchLanguage') || 'Rechercher...'}" class="input-field w-full text-sm" oninput="langPickerFilter(this.value)" value="${state.langPickerSearch || ''}" autofocus /></div>
 ...
 <span class="text-sm font-medium">${lang}</span></button>
 `).join('')}
...
export function renderLanguageLevelModal(state) {
 ...
 <div class="text-[10px] text-slate-400">${l.desc}</div></div></button>
 `).join('')}
 </div></div></div>
 `
}
...
export function renderProfileSubTabs(activeTab) {
 const tabs = [
...
export function renderProfilTab(state) {
 ...
 ${svgBio}
 <span class="text-xs font-semibold text-slate-400 uppercase tracking-wide">${t('bio') || 'Bio'}</span></div>
 ${bio
 ...
 <div class="flex items-center gap-2 mb-2">
 ${svgLang}
 <span class="text-xs font-semibold text-slate-400 uppercase tracking-wide">${t('languages') || 'Langues'}</span></div>
 ${langs.length > 0
 ...
 <span class="flex gap-0.5 ml-0.5">${levelDot(l.level)}</span></span>
 `).join('')}
 </div>`
 ...
 <span class="ml-auto text-xs text-primary-400">${t('seeAll') || 'Voir tout'} →</span></button></div>
 ` : ''}
...
function renderClickableStats(state) {
 ...
 <div class="text-[10px] text-slate-600 mt-0.5">${t('tapForDetails') || 'Voir détails'} →</div></button>
 <button
 ...
 <div class="text-[10px] text-slate-600 mt-0.5">${t('tapForDetails') || 'Voir détails'} →</div></button></div>
 `
}
...
function renderVerificationCard(state) {
 ...
 <span class="text-[10px] text-center leading-tight ${s.done ? 'text-emerald-400' : 'text-slate-500'}">${s.label}</span></div>
 `).join('')}
 </div></div>
 `
}
...
function renderMySpotsList(state) {
 ...
 <div class="w-3 h-3 rounded-full" style="background:${freshness.hexColor}"></div></div>
 <div class="flex-1 min-w-0">
 ...
 <span class="text-xs text-slate-500">(${g.spots.length} ${t('spots') || 'spots'})</span></div>
 <div class="space-y-2">
 ...
 <div class="text-[10px] text-slate-400">${t('spots') || 'spots'}</div></div>
 <div class="bg-slate-800/50 rounded-lg p-2">
 <div class="text-lg font-bold text-white">${uniqueCountries.size}</div>
 <div class="text-[10px] text-slate-400">${t('countries') || 'pays'}</div></div>
 <div class="bg-slate-800/50 rounded-lg p-2">
 <div class="text-lg font-bold text-emerald-400">${totalValidations}</div>
 <div class="text-[10px] text-slate-400">${t('totalValidations') || 'validations reçues'}</div></div>
 <div class="bg-slate-800/50 rounded-lg p-2">
 <div class="text-sm font-bold text-amber-400 truncate">${bestSpotName ? escapeHTML(bestSpotName) : '...'}</div>
 <div class="text-[10px] text-slate-400">${t('bestSpot') || 'Meilleur spot'}</div></div></div></div>
...
function renderMyValidationsList(state) {
 ...
 <p class="text-slate-400 text-sm">${t('noValidationsYet') || 'Aucune validation encore. Valide des spots que tu as utilisés !'}</p></div>`
 : `<div class="space-y-2">
...
function renderMyCountriesList(state) {
 ...
 ? `<div class="card p-6 text-center">
 <p class="text-slate-400 text-sm">${t('noCountriesYet') || 'Aucun pays encore. Commence à bouger !'}</p></div>`
 ...
 <div class="text-[10px] text-slate-400">${t('spotsUsedIn') || 'spots utilisés'}</div></div></div>
 `
 }).join('')}
 </div>`
 }
...
export function renderRoadmapTab(_state) {
 ensureRoadmapTotals()
 ...
 <div>
 <div class="flex items-center justify-between mb-2">
 <h2 class="text-lg font-bold">${icon('rocket', 'w-5 h-5 text-amber-400 inline-block mr-1')} ${t('roadmapTitle') || 'Roadmap communautaire'}</h2></div>
 <div class="card p-3 mb-4 border-amber-500/20 bg-amber-500/5">
 <p class="text-slate-300 text-xs leading-relaxed">${t('roadmapCommunityIntro') || "Vote pour les features que tu veux en premier ! Plus de votes = plus de priorité."}</p></div>
...
export function renderReglagesTab(state) {
 ...
 <div class="text-[11px] text-slate-500">${sub}</div></div></div>
 <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#475569" stroke-width="2"
 style="transition:transform .2s;transform:rotate(${isOpen ? '90' : '0'}deg)"><path d="M9 18l6-6-6-6"/></svg></button>
...
function renderOfflineManagerCard(_state) {
 ...
 <span class="text-[10px] text-primary-400">${icon('download', 'w-3.5 h-3.5')}</span></button>
 `).join('')
...
function renderAppearanceCard(state) {
 ...
 <span class="text-sm">${t('language') || 'Langue'}</span></div>
...
function renderNotificationsCard(state) {
 ...
 <span class="text-sm">${t('notifications') || 'Notifications'}</span></div>
...
function renderPrivacyCard(_state) {
 ...
 <span class="text-xs text-slate-500">${t('sharePastTripsDesc') || 'Tes itinéraires et dates visibles par les autres'}</span></div></div>
 ...
 <span class="text-sm">${t('blockedUsers') || 'Utilisateurs bloqués'}</span></div>
 ${icon('chevron-right', 'w-4 h-4 text-slate-500')}
 </button>
 <button
 onclick="openMyData()"
 ...
 <span class="text-sm">${t('myData') || 'Mes données'} (RGPD)</span></div>
 ${icon('chevron-right', 'w-4 h-4 text-slate-500')}
 </button></div>
 `
}
...
function renderActionsCard(state) {
 ...
 <span class="text-sm">${t('logout') || 'Se déconnecter'}</span></button>
 <button
 onclick="openDeleteAccount()"
 class="w-full flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-red-500/10 transition-colors text-red-400"
 >
 ${icon('trash', 'w-4 h-4')}
 <span class="text-sm">${t('deleteAccount') || 'Supprimer le compte'}</span></button>
 ` : `
 <button
 onclick="openAuth()"
...
export function renderVersionReset() {
 return `
...
window.showCityPageDemo = () => {
 ...
 <div class="flex items-start gap-2.5 text-left mb-2"><span class="text-[1.1rem] shrink-0 mt-px"></span><span class="text-[0.78rem] text-slate-300 leading-[1.4]">Chaque ville a sa propre page avec spots classés par direction, temps d'attente moyen et meilleure heure</span></div>
 ...
 <div class="flex items-start gap-2.5 text-left mb-2"><span class="text-[1.1rem] shrink-0 mt-px"></span><span class="text-[0.78rem] text-slate-300 leading-[1.4]">Lois locales sur l'auto-stop, numéros d'urgence et phrases utiles dans la langue du pays</span></div>
 <div class="flex items-start gap-2.5 text-left mb-2"><span class="text-[1.1rem] shrink-0 mt-px"></span><span class="text-[0.78rem] text-slate-300 leading-[1.4]">Stats complètes : meilleure saison, types de spots, temps d'attente par moment de la journée</span></div></div>
 <button onclick="startCityPageDemo()" class="bg-gradient-to-br from-amber-300 to-amber-600 text-dark-primary font-bold border-none px-7 py-3 rounded-xl text-[0.9rem] cursor-pointer">${escapeHTML(t('cityDemoIntroBtn') || 'Découvrir la démo')}</button></div>

 <!-- Demo screen (hidden initially) -->
 <div id="city-demo-main-screen" class="hidden"></div></div>
 `
...
window.startCityPageDemo = () => {
 ...
 <div class="bg-dark-secondary rounded-lg p-2 text-center"><div class="text-[1.1rem] font-extrabold text-amber-300">847</div><div class="text-[0.5rem] text-slate-500 uppercase tracking-wide">Lifts</div></div></div>
 ...
 <div class="bg-dark-secondary rounded-lg p-1.5 text-center"><div class="text-[0.8rem] font-extrabold text-emerald-500">~25&rsquo;</div><div class="text-[0.5rem] text-slate-500">Attente</div></div></div>
 ...
 <span class="cd-tab" onclick="switchCityDemoTab(this,'loi')" role="button" tabindex="0"> Loi</span>
 <span class="cd-tab" onclick="switchCityDemoTab(this,'pratique')" role="button" tabindex="0"> Pratique</span></div>
 ...
 <div class="bg-dark-secondary rounded-md px-[3px] py-[5px] text-center"><div class="text-[0.9rem]"></div><div class="text-[0.5rem] text-slate-500">Nuit</div><div class="text-[0.7rem] font-bold text-amber-300">~55&rsquo;</div></div></div></div>
 ...
 <div class="flex flex-wrap gap-2 text-[0.6rem] text-slate-400"><span><span class="w-1.5 h-1.5 rounded-full inline-block mr-[3px] align-middle bg-emerald-500"></span>Sortie 40%</span><span><span class="w-1.5 h-1.5 rounded-full inline-block mr-[3px] align-middle bg-red-500"></span>Station 30%</span><span><span class="w-1.5 h-1.5 rounded-full inline-block mr-[3px] align-middle bg-blue-500"></span>Route 18%</span><span><span class="w-1.5 h-1.5 rounded-full inline-block mr-[3px] align-middle bg-slate-400"></span>Autre 12%</span></div></div></div>
 ...
 <div class="bg-dark-secondary rounded-lg px-2.5 py-2 mb-[5px] flex gap-2 items-center"><div class="text-[1.3rem]"></div><div class="flex-1"><div class="text-xs font-semibold">Generator Paris</div><div class="text-[0.6rem] text-slate-500">Colonel Fabien · Rooftop</div></div><div class="text-[0.7rem] font-bold text-emerald-500">28€</div></div></div>
 <div class="bg-dark-secondary border-l-2 border-l-amber-300 rounded-r-lg px-2.5 py-2 text-[0.68rem]">Prix = dortoir, basse saison. Haute saison : +30-50%</div></div>
 ...
 <div class="bg-dark-secondary rounded-lg px-2.5 py-2 mb-[5px]"><div class="text-xs font-semibold"> Nuit Blanche</div><div class="text-[0.6rem] text-amber-300">3 oct 2026</div><div class="text-[0.6rem] text-slate-400 mt-0.5">Art toute la nuit — transports gratuits</div></div></div>
 <div class="mt-2.5"><div class="text-xs font-bold text-amber-300 mb-1.5 flex items-center gap-[5px]"><span class="text-[0.9rem]"></span> Meilleure saison</div>
 <div class="grid grid-cols-4 gap-1">
 <div class="bg-dark-secondary rounded-md px-[3px] py-[5px] text-center"><div class="text-[0.9rem]"></div><div class="text-[0.5rem] text-slate-500">Hiver</div><div class="text-[0.7rem] font-bold text-red-500">2/5</div></div>
 ...
 <div class="bg-emerald-500/[0.06] border border-emerald-500/30 rounded-md px-[3px] py-[5px] text-center"><div class="text-[0.9rem]"></div><div class="text-[0.5rem] text-slate-500">Été</div><div class="text-[0.7rem] font-bold text-emerald-500">5/5</div></div>
 <div class="bg-dark-secondary rounded-md px-[3px] py-[5px] text-center"><div class="text-[0.9rem]"></div><div class="text-[0.5rem] text-slate-500">Auto.</div><div class="text-[0.7rem] font-bold text-amber-300">3/5</div></div></div></div></div>
 ...
 <div class="bg-dark-secondary rounded-lg px-2.5 py-2 mb-[5px] text-xs leading-[1.4]"> <strong>Périph</strong> — interdit, mais portes de Paris OK</div>
 <div class="bg-dark-secondary rounded-lg px-2.5 py-2 mb-[5px] text-xs leading-[1.4]"> <strong>Nationales</strong> — aucune restriction</div></div>
 ...
 <div class="bg-dark-secondary rounded-lg p-1.5 text-center"><div class="text-[0.9rem]"></div><div class="text-[0.5rem] text-slate-500">18</div></div></div></div></div>

 <!-- Panel: Pratique -->
 <div data-cd-panel="pratique" class="hidden">
 <div class="mt-2.5"><div class="text-xs font-bold text-amber-300 mb-1.5 flex items-center gap-[5px]"><span class="text-[0.9rem]"></span> Phrases utiles</div>
 ...
 <div class="bg-dark-secondary rounded-md px-2 py-1.5 mb-1 flex justify-between items-center"><div class="text-[0.72rem] font-semibold">Merci pour le lift !</div><div class="text-[0.65rem] text-slate-500">Thanks for the ride!</div></div></div>
 ...
 <div class="bg-dark-secondary rounded-lg p-2 text-center"><div class="text-[0.85rem] font-extrabold text-amber-300">~15€</div><div class="text-[0.5rem] text-slate-500">Budget/jour</div></div></div>
 <div class="bg-dark-secondary rounded-lg px-2.5 py-2 text-[0.68rem] mt-1 leading-[1.4]"> Café : 1.50€ · Baguette : 1.10€ · Métro : 2.15€ · Pizza : 8€</div></div>
 ...
 <div class="flex items-center gap-2 bg-dark-secondary rounded-lg px-2.5 py-1.5 mb-1"><div class="text-[0.8rem] font-extrabold text-amber-300 min-w-[20px]">→</div><div class="flex-1"><div class="text-[0.72rem] font-semibold">Nantes</div><div class="text-[0.58rem] text-slate-500">385 km · 31 spots</div></div><div class="text-[0.72rem] font-bold text-emerald-500">~30&rsquo;</div></div></div></div></div>
 `
}
