...
export function renderSpotDetail(state) {
 ...
 <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#e2e8f0" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button></div>
 ...
 data-favorite-btn
 class="w-8 h-8 bg-[rgba(15,21,32,0.7)] backdrop-blur-md rounded-full flex items-center justify-center border-none cursor-pointer">
 <svg width="18" height="18" viewBox="0 0 24 24" fill="${isFav ? '#f59e0b' : 'none'}" stroke="${isFav ? '#f59e0b' : '#e2e8f0'}" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg></button>
 <button onclick="event.stopPropagation();openShareCard()" type="button"
 class="w-8 h-8 bg-[rgba(15,21,32,0.7)] backdrop-blur-md rounded-full flex items-center justify-center border-none cursor-pointer">
 <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#e2e8f0" stroke-width="2"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg></button></div>

 <!-- Type badge (bottom-left) -->
 <div class="absolute bottom-3 left-3 z-2">
 <span class="text-[11px] text-primary-500 bg-[rgba(15,21,32,0.8)] px-2.5 py-1 rounded-full">${renderSubtitleType(spot)}</span></div></div>
 ...
 ${spot.needsReview ? `<div class="mx-3 mt-2 bg-blue-500/20 border border-blue-500/40 rounded-xl px-3 py-2 flex items-center gap-2"><span class="text-base"></span><span class="text-xs text-blue-300 font-medium">${t('spotNeedsReview') || 'Les informations de ce spot sont peut-être inexactes.'}</span></div>` : ''}
 ...
 <div class="text-[10px] text-slate-500">${t('waitTimeLabel') || 'Attente'}</div></div>
 <div class="w-px bg-slate-800"></div>
 ...
 <div class="text-[10px] text-slate-500">${t('successes') || 'Réussites'}</div></div>
 ...
 <div class="text-[10px] text-slate-500">${t('failures') || 'Échecs'}</div></div></div>
 ...
 `).join('')}
 ` : ''}
 <div class="text-center text-[10px] text-[#475569] pt-1.5 cursor-pointer" onclick="this.parentElement.classList.add('hidden')" role="button" tabindex="0">${t('close') || 'Fermer'} ▲</div></div>
 ...
 <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="${spot.streetViewVerified ? '#22c55e' : '#64748b'}" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg></div></div></div>
 ` : ''}
 ...
 <span class="text-[10px] text-[#475569] font-normal">${t('validateSubtitle') || 'Le spot est toujours là'}</span></button>
 ...
 <span class="text-[10px] text-[rgba(15,21,32,0.6)] font-normal">${t('experienceSubtitle') || "J'ai fait du stop ici"}</span></button></div>
 ...
 <div class="text-xs text-slate-200">${spot.lastValidated ? formatRelativeDate(spot.lastValidated) : (spot.lastUsed ? formatRelativeDate(spot.lastUsed) : '—')}${displayName(spot.liveLastValidatedBy || spot.lastValidatedBy)}</div></div>
 <div class="flex-1 bg-[#161b28] rounded-lg py-2 px-2.5">
 <div class="text-[10px] text-slate-500 uppercase">${t('lastTest') || 'Dernière utilisation'}</div>
 <div class="text-xs text-slate-200">${(spot.liveLastTested || spot.lastTested) ? formatRelativeDate(spot.liveLastTested || spot.lastTested) : '—'}${displayName(spot.liveLastTestedBy || spot.lastTestedBy)}</div></div></div>
 ...
 <div class="text-[10px] text-slate-500 ml-auto">${formatRelativeDate(spot.lastGpsVerified)}${spot.lastGpsDistance != null ? ` · ${spot.lastGpsDistance}m` : ''}${displayName(spot.lastGpsVerifiedBy)}</div></div></div>` : ''}
 ...
 <div class="text-[10px] text-slate-500">${t('safety') || 'Sécurité'}</div></div>
 <div class="flex-1 bg-[#161b28] rounded-lg p-2.5 text-center">
 <div class="text-lg font-semibold text-blue-500">${traffic || '—'}${traffic ? '/5' : ''}</div>
 <div class="text-[10px] text-slate-500">${t('traffic') || 'Trafic'}</div></div>
 <div class="flex-1 bg-[#161b28] rounded-lg p-2.5 text-center">
 <div class="text-lg font-semibold text-purple-500">${access || '—'}${access ? '/5' : ''}</div>
 <div class="text-[10px] text-slate-500">${t('accessibility') || 'Acces'}</div></div></div>
 ` : ''}
 ...
 class="bg-[#161b28] border border-slate-700 text-slate-400 py-2 px-3 rounded-lg text-[11px] cursor-pointer whitespace-nowrap">\ud83d\uddfa ${t('map') || 'Carte'}</button></div></div>
...
function renderPhotoHero(spot) {
 ...
 <span class="text-[11px] text-[#475569]" id="spot-hero-loading">${t('loadingStreetPhotos') || 'Chargement des photos...'}</span></div>`
}
