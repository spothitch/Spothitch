...
const FEATURES = {
 ...
 { id: 'filters', emoji: '', nameKey: 'fbFeatFilters' },
 { id: 'create-spot', iconName: 'map-pin', nameKey: 'fbFeatCreateSpot' },
 { id: 'route', iconName: 'map', nameKey: 'fbFeatRoute' },
 { id: 'gas-stations', iconName: 'fuel', nameKey: 'fbFeatGasStations' },
 { id: 'spot-detail', iconName: 'bar-chart-3', nameKey: 'fbFeatSpotDetail' },
 { id: 'offline-map', emoji: '', nameKey: 'fbFeatOfflineMap', status: 'coming' },
 { id: 'en-route', iconName: 'eye', nameKey: 'fbFeatEnRoute', status: 'coming' },
 { id: 'city-pages', emoji: '', nameKey: 'fbFeatCityPages', status: 'coming' },
 ],
 voyage: [
 { id: 'trip-planner', iconName: 'compass', nameKey: 'fbFeatTripPlanner' },
 { id: 'badges', iconName: 'trophy', nameKey: 'fbFeatBadges' },
 { id: 'guides', emoji: '', nameKey: 'fbFeatGuides' },
 { id: 'challenges', iconName: 'target', nameKey: 'fbFeatChallenges' },
 { id: 'quiz', iconName: 'brain', nameKey: 'fbFeatQuiz' },
 { id: 'journal', emoji: '', nameKey: 'fbFeatJournal', status: 'coming' },
 { id: 'hostels', iconName: 'building', nameKey: 'fbFeatHostels', status: 'coming' },
 { id: 'leagues', iconName: 'medal', nameKey: 'fbFeatLeagues', status: 'coming' },
 ],
 social: [
 { id: 'friends', emoji: '', nameKey: 'fbFeatFriends' },
 { id: 'private-messages', iconName: 'message-circle', nameKey: 'fbFeatPrivateMessages' },
 { id: 'feed', emoji: '', nameKey: 'fbFeatFeed' },
 { id: 'companion-route', iconName: 'handshake', nameKey: 'fbFeatCompanionRoute' },
 { id: 'events', iconName: 'party-popper', nameKey: 'fbFeatEvents', status: 'coming' },
 { id: 'groups-races', iconName: 'users', nameKey: 'fbFeatGroupsRaces', status: 'coming' },
 ],
 profil: [
 { id: 'my-profile', iconName: 'user', nameKey: 'fbFeatMyProfile' },
 { id: 'identity-verify', emoji: '', nameKey: 'fbFeatIdentityVerify' },
 { id: 'settings', emoji: '', nameKey: 'fbFeatSettings' },
 { id: 'thumbs-partners', emoji: 'thumbs-up', nameKey: 'fbFeatThumbsPartners', status: 'coming' },
 { id: 'tech-improvements', iconName: 'rocket', nameKey: 'fbFeatTechImprovements', status: 'coming' },
 ],
 securite: [
...
const REACTION_META = {
 like: { emoji: 'thumbs-up', color: '#22c55e', bg: 'rgba(34,197,94,0.15)' },
 love: { iconName: 'heart', color: '#ec4899', bg: 'rgba(236,72,153,0.15)' },
 bug: { emoji: '', color: '#ef4444', bg: 'rgba(239,68,68,0.15)' },
 idea: { iconName: 'lightbulb', color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' },
...
function pctBar(count, total, color) {
 ...
 <span class="text-xs text-slate-300 w-14 text-right">${count} (${pct}%)</span></div>`
}
...
function renderFeedbackTab(state) {
 ...
 <div class="text-xs text-slate-400">${t('adminFbTotalReviews') || 'Avis reçus'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-purple-400">${a.uniqueUsers}</div>
 <div class="text-xs text-slate-400">${t('adminFbUniqueUsers') || 'Utilisateurs uniques'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-emerald-400">${a.satisfactionPct}%</div>
 <div class="text-xs text-slate-400">${t('adminFbSatisfaction') || 'Satisfaction'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-red-400">${a.bugCount}</div>
 <div class="text-xs text-slate-400">${t('adminFbBugsReported') || 'Bugs signalés'}</div></div></div>`
 ...
 <span class="text-xs text-emerald-400 font-bold"> ${f.score}</span></div>`).join('')
 ...
 <span class="text-sm">${featureName(f.fid)}</span>
 <span class="text-xs text-red-400 font-bold"> ${f.score}</span></div>`).join('')
 : `<p class="text-xs text-slate-500 italic">${t('adminFbNoData') || 'Aucune donnée'}</p>`
 ...
 <span class="text-sm">${featureName(f.fid)}</span>
 <span class="text-xs text-amber-400 font-bold">${f.docCount} avis</span></div>`).join('')
 : `<p class="text-xs text-slate-500 italic">${t('adminFbNoData') || 'Aucune donnée'}</p>`
 ...
 <span class="text-sm">${featureName(f.fid)}</span>
 <span class="text-xs text-pink-400 font-bold">${f.likeScore}</span></div>`).join('')
 : `<p class="text-xs text-slate-500 italic">${t('adminFbNoData') || 'Aucune donnée'}</p>`
 ...
 <p class="text-sm text-slate-200">${escapeHTML(d.comment || '')}</p></div>`
 }).join('')
 ...
 <div class="max-h-60 overflow-y-auto">${commentsHtml}</div></div>
...
function renderSentryTab(state) {
 ...
 <div class="text-xs text-slate-400">${t('adminSentryOpen') || 'Non résolues'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-amber-400">${issues.length}</div>
 <div class="text-xs text-slate-400">${t('adminSentryTotal') || 'Total'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-purple-400">${recent24h.length}</div>
 <div class="text-xs text-slate-400">${t('adminSentry24h') || '24h'}</div></div></div>`
 ...
 ${statusBadge}
 <span class="text-xs text-slate-500">${dateStr}</span>
 <span class="text-xs text-slate-500">#${issue.number || ''}</span></div></div>
 ...
 <button onclick="loadAdminSentry()" class="text-xs text-amber-400 hover:underline">${t('adminSentryRefresh') || 'Rafraîchir'}</button></div>
 <div class="max-h-96 overflow-y-auto">${issueListHtml}</div></div>`
}
...
function renderToolsTab(state) {
 ...
 <div class="text-xs text-slate-400">admin.spothitch.com</div></div></a>
 ...
 <div class="text-xs text-slate-400">${t('points') || 'Pouces'}</div></div>
 <div>
 <div class="font-bold text-emerald-400">${state.level || 1}</div>
 <div class="text-xs text-slate-400">${t('level') || 'Niveau'}</div></div>
 <div>
 <div class="font-bold text-purple-400">${state.skillPoints || 0}</div>
 <div class="text-xs text-slate-400">${t('skillPoints') || 'Skill pts'}</div></div>
 <div>
 <div class="font-bold text-amber-400">${state.thumbs || 0}</div>
 <div class="text-xs text-slate-400">${t('thumbs') || 'Pouces'}</div></div></div></div>
 ...
 <button onclick="adminAddPoints(100)" class="btn-secondary text-sm py-2">+100 pts</button>
 <button onclick="adminAddPoints(1000)" class="btn-secondary text-sm py-2">+1000 pts</button>
 <button onclick="adminAddSkillPoints(5)" class="btn-secondary text-sm py-2">+5 Skill pts</button>
 ...
 <button onclick="adminMaxStats()" class="btn-primary text-sm py-2">MAX ALL</button></div></div>
 ...
 <button onclick="changeTab('profile'); closeAdminPanel();" class="px-3 py-1.5 rounded-full bg-emerald-500/20 text-emerald-400 text-sm hover:bg-emerald-500/30">${t('profile') || 'Profil'}</button></div></div>`
}
...
function renderGuideTipsTab(state) {
 ...
 hitchhiking: 'thumbs-up', safety: '', laws: '', language: '',
 budget: '', culture: '', transport: '',
 }
 ...
 <button onclick="adminRejectGuideTipAction('${escapeJSString(tip.id)}')" class="text-xs px-2 py-1 rounded bg-red-500/20 text-red-400 hover:bg-red-500/30"> ${t('adminGuideTipReject') || 'Rejeter'}</button></div></div></div></div>`
 }).join('')
 ...
 <div class="text-xs text-slate-400">${t('adminReportsPending') || 'En attente'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-slate-400">${[...new Set(tips.map(t => t.countryCode))].length}</div>
 <div class="text-xs text-slate-400">${t('adminGuideTipsCountries') || 'Pays'}</div></div></div>

 <div class="card p-3 mb-4">
 <div class="flex items-center justify-between mb-2">
 <h4 class="text-sm font-bold text-slate-200">${t('adminGuideTipsList') || 'Contributions en attente'} (${tips.length})</h4>
 <button onclick="loadAdminGuideTips()" class="text-xs text-amber-400 hover:underline">${t('adminSentryRefresh') || 'Rafraîchir'}</button></div>
 <div class="max-h-96 overflow-y-auto">${tipsHtml}</div></div>`
}
...
function renderReportsTab(state) {
 ...
 const REASON_ICONS = {
 misplaced: '', inaccurate: '', dangerous: '', inappropriate: '',
 ...
 <div class="text-xs text-slate-400">${t('adminReportsConfirmed') || 'Confirmés'}</div></div>
 <div class="card p-3 text-center">
 <div class="text-2xl font-bold text-slate-400">${dismissed.length}</div>
 <div class="text-xs text-slate-400">${t('adminReportsDismissed') || 'Rejetés'}</div></div></div>`
 ...
 <button onclick="loadAdminReports()" class="text-xs text-amber-400 hover:underline">${t('adminSentryRefresh') || 'Rafraîchir'}</button></div>
 <div class="max-h-96 overflow-y-auto">${reportListHtml}</div></div>`
}
...
renderReportsTab = function(state) {
 ...
 const ICONS = { misplaced: '', inaccurate: '', dangerous: '', inappropriate: '', duplicate: '', closed: '', spam: '', harassment: '', fake: '', hate: '', other: 'ℹ️', guide_error: '' }
 ...
 else if (r.reason === 'inaccurate') ab = `<div class="flex gap-1.5 mt-2"><button onclick="adminConfirmReport('${escapeJSString(r.id)}','${escapeJSString(r.targetId)}','needsReview')" class="text-xs px-2 py-1 rounded bg-amber-500/20 text-amber-400"> À vérifier</button><button onclick="adminConfirmReport('${escapeJSString(r.id)}','${escapeJSString(r.targetId)}','hide')" class="text-xs px-2 py-1 rounded bg-orange-500/20 text-orange-400">Masquer</button><button onclick="adminDismissReport('${escapeJSString(r.id)}')" class="text-xs px-2 py-1 rounded bg-slate-500/20 text-slate-400">Rejeter</button></div>`
...
window.adminAddPoints = (amount) => {
 ...
 window.showToast?.(t('pointsAdded')?.replace('{amount}', amount) || `+${amount} pts ajoutés`, 'success')
}

window.adminAddSkillPoints = (amount) => {
 const state = getState()
 setState({ skillPoints: (state.skillPoints || 0) + amount })
 window.showToast?.(t('skillPointsAdded')?.replace('{amount}', amount) || `+${amount} skill pts ajoutés`, 'success')
}
