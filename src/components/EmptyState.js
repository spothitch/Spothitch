...
export function renderEmptyState(type, opts = {}) {
 ...
 <span class="flex justify-center mb-3">${icon("help-circle", "w-10 h-10 text-slate-400")}</span>
 <p class="text-sm text-slate-400">${t('nothingHere')}</p></div>
 `
 }
 ...
 <span class="flex justify-center mb-2">${icon(state.iconName || 'circle', 'w-10 h-10 text-slate-400')}</span>
